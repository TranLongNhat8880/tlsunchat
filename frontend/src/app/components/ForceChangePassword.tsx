...
export function ForceChangePassword({ onSuccess, onLogout }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password === '123456') {
      setError('Bạn không được tiếp tục sử dụng mật khẩu mặc định (123456)');
      return;
    }
    if (password !== confirm) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 border border-green-100">
        ...
        <p className="text-center text-gray-500 mb-8 text-sm">
          Bạn đang sử dụng mật khẩu mặc định. Vui lòng đổi mật khẩu mới để bảo vệ tài khoản của bạn.
        </p>
